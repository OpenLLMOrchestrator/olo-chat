...
- **Left panel** — Tenant dropdown, section navigation (**Chat**, **Knowledge**, **Documents**). Under Chat: **Conversation** (single submenu). Under Knowledge: sub-options (Sources, Create new, Status). Under Documents: Upload / manage raw files.
- **Center** — **Chat**: conversation view (messages, input, send, run events). **Knowledge**: Sources / Create new / Status views. **Documents**: Upload and manage raw files (knowledge source + file/folder, Start RAG).
- **Tools panel** — **Chat**: Conversation sidebar (**Queue** dropdown, **Pipeline** dropdown, New chat, sessions list, Delete). **Knowledge**: List of knowledge sources (second panel). Hidden for Documents.
...
| **Section navigation** | Sections: **Chat**, **Knowledge**, **Documents**. Chat has a single sub-option **Conversation** (URL `/chat/conversation`). Knowledge has sub-options: Sources, Create new, Status. Documents has: Upload / manage raw files. Clicking a section/sub updates the URL path (e.g. `/chat/conversation`, `/knowledge/sources`, `/documents/upload`). |
| **Chat → Conversation** | Under Chat, the left panel shows only **Conversation** as the submenu. Workflow queues are selected in the **Conversation panel** (Tools panel) via the **Queue** dropdown, not in the left panel. |
...
| Feature | Description |
|--------|-------------|
| **Sub-options** | **Sources** — List of knowledge sources is shown in the second panel (Tools). **Create new** — Create new knowledge source (placeholder; wire to API). **Status** — Indexed, processing (placeholder; wire to API). |
| **Header** | Title “Knowledge” and subtitle “→ {currentLabel}” (e.g. Sources, Create new, Status). |
| **Tools panel** | When section is Knowledge, the second panel shows “Knowledge sources” and a list of knowledge sources (placeholder until API). |
...
| **Content** | Dropdown of existing knowledge sources (from `VITE_RAG_OPTIONS`), or text field to use a different one; file/folder selector with drag-and-drop; **Start RAG** button fires the upload API (queue/pipeline from `VITE_RAG_QUEUE`, `VITE_RAG_PIPELINE`). |
